...
```go
// UNCLEAR: logic buried inside an expression
status := map[bool]string{true: "active", false: "inactive"}[user.IsActive]

// CLEAR: readable guard-style
...
```go
// UNCLEAR: all logic crammed into one line with an IIFE
counts := func() map[string]int { m := map[string]int{}; for _, i := range items { m[i.ID]++ }; return m }()
...
When simplifying Go or Python code, read `references/code-simplification-patterns.md` for before/after examples of common simplifications in that language.
